...
- `sandbox-bootstrap.ts` owns low-level Sandbox SDK, clone/fetch/install,
  runner-health, and the archive-backed backup/restore wrappers.
...
|---|---|---|
| `projects` | User-owned GitHub project and sandbox lifecycle | Status is provisioning/ready/failed; env vars and opaque archive IDs never return in normal project responses |
