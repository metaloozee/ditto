...
`sleepAfter = "10m"`; internet left enabled for legacy instances) with a catch-all outbound handler
registry, re-exports `ContainerProxy`, and calls `proxyToSandbox(request, env)`
before the TanStack Start fetch handler. Preview traffic timestamps are stored
only in that Durable Object. `scheduled()` drains the runtime-work queue once a
minute. Builders and new workspace-session
...
- `session-preview.ts` owns session website preview. Dedicated session sandboxes
  use one well-known localhost port and a stable `exposePort` capability URL.
  Preview traffic observation lives in Sandbox Durable Object storage, not D1.
  Legacy shared-sandbox sessions still use the project D1 lease and
  `10000..10031` allocator. Preview URLs and tokens are never persisted.
- `workspace-runtime-capacity.ts` is internal to `WorkspaceRuntime`. It owns the
  FIFO runtime-work queue, 20-global / 2-per-user running-slot CAS, cancellation,
  queue expiry, and drain. Callers submit a serializable work intent and receive
  a receipt.
...
| `workspace_runtime_work` | Durable runtime-work queue | FIFO sequence; status queued/leased/running/complete/failed/cancelled; identifiers and bounded payload only |
| `workspace_capacity_leases` | Running capacity slots | Unexpired rows count toward the 20-global / 2-per-user limits |
...
The initial user row, pending assistant row, and runtime-work row are inserted
together before capacity or provision. Immediate-capacity runs stream over SSE;
queued runs persist the same settlement when the drain later executes them.
Every started assistant reaches `complete` or `failed` before its turn or the
outer run settles. Follow-ups cleared before start never create rows. Queue
expiry and cancellation mark the assistant failed and keep the user message.
