        ...
        .query(async ({ ctx, input }) => {
            ...
            const events = selectedSession
                ? await db
                      ...
                      .orderBy(
                          desc(agentRunEvents.createdAt),
                          desc(agentRunEvents.id),
                      )
                      .limit(100)
                : [];
                ...
                if (input.isMutating) {
                    ...
                    if (!lockedProject) {
                        ...
                        throw new TRPCError({
                            code: "CONFLICT",
                            message:
                                "Another agent run is already editing this project.",
                        });
                    }
