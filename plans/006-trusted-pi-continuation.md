...
Move full Pi 0.80.10 into the trusted Node image for isolated target sessions. Preserve the fixed model, supported thinking levels, compaction, image-owned Git metadata tools and queued follow-ups. Repository execution stays remote.
...
An async subscription is not a persistence barrier. Installed `packages/sandbox-runner/node_modules/@earendil-works/pi-coding-agent/dist/core/session-manager.d.ts:36-45` requires compaction `firstKeptEntryId`; its `static inMemory` declaration accepts only cwd/options. Do not assume an entries overload or another compaction format. Global documentation research was reported by the earlier draft, not reverified in this editorial pass. The earlier drafter also reported an agent-core resolution failure; 001 must establish the actual resolved dependency graph and runtime behavior.
...
7. Reconstruct only supported positions. Rebuild validated JSONL in the brain's private state directory, select recorded leaf and restore explicit settings. Completed logical tools return stored results; not-yet-admitted tools can dispatch under a new attempt; admitted unresolved shell work becomes unknown. Reconcile live executor incarnation first. Read-only retry creates a new observation; structured write recovery uses its expected-content check. No automatic shell compensation.
